...
<?= $this->include('App\Views\Permission\create') ?>
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <div class="float-right">
                    <div class="btn-group">
                        <button type="button" class="btn btn-sm btn-block btn-primary" id="create-book" data-toggle="modal" data-target="#modal-create-permission"><i class="fa fa-plus"></i>
...
<script>
    ...
    $(document).on('click', '.btn-edit', function(e) {
        e.preventDefault();
        $.ajax({
            url: `<?= route_to('permission') ?>/${$(this).attr('data-id')}/edit`,
            method: 'GET',
    ...
    $(document).on('click', '#btn-update-permission', function(e) {
        ...
        $.ajax({
            url: `<?= route_to('permission') ?>/${ $('#permission_id').val() }`,
            method: 'PUT',
    ...
    $(document).on('click', '.btn-delete', function(e) {
        Swal.fire({
            ...
            .then((result) => {
                if (result.value) {
                    $.ajax({
                        url: `<?= route_to('permission') ?>/${$(this).attr('data-id')}`,
                        method: 'DELETE',
                    }).done((data, textStatus, jqXHR) => {
                        Toast.fire({
                            icon: 'success',
