<!-- Include -->
<?= $this->include('App\Views\load\nestable') ?>
<?= $this->include('App\Views\load\select2') ?>
<?= $this->include('App\Views\load\iconpicker') ?>
<!-- Extend from layout index -->
<?= $this->extend('App\Views\layout\index') ?>

<!-- Section content -->
<?= $this->section('content') ?>
<?= $this->include('App\Views\Menu\update') ?>
<style>
    .fade.in {
        opacity: 1;
    }
...
<div class="row">
    ...
    <div class="col-lg-7">
        <div class="card card-primary card-outline">
            ...
            <div class="card-body">
                <form action="<?= route_to('menu') ?>" method="post" class="form-horizontal">
                    <?= csrf_field() ?>
...
<script>
    $(function() {
        ...
        function menu() {
            $.get("<?= base_url('menu') ?>", function(response) {
                $('.dd').nestable({
                    maxDepth: 2,
                    json: response.data,
        ...
        $(document).on('click', '#btn-edit', function(e) {
            ...
            $.ajax({
                url: `<?= route_to('menu') ?>/${$(this).attr('data-id')}/edit`,
                method: 'GET',
                dataType: 'JSON',
        ...
        $(document).on('click', '#btn-delete', function(e) {
            Swal.fire({
                ...
                .then((result) => {
                    if (result.value) {
                        $.ajax({
                            url: `<?= route_to('menu') ?>/${$(this).attr('data-id')}`,
                            method: 'DELETE',
                        }).done((data, textStatus, jqXHR) => {
                            Toast.fire({
                                icon: 'success',
